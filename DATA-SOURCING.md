# Data sourcing status for the locked data points
...
### D. Investigated and found NOT viable via OSM/World Bank — 3 points (one since shipped from a different source)

**Senior Care Facilities.** SHIPPED on 16 Sep 2026 via the Dubai Health Authority Sheryan licensed-facility register (212 facilities, 52 communities) - the OSM route below was not viable, but the DHA register was.

OSM check, kept for the record: previously recorded as an OSM quick win. It is not.
...
*Update 21 Sep 2026:* production has since unlocked Senior Care Facilities (DHA register) and added ten further data points; the per-point verdicts above stand for everything still locked. The five mis-attributed live data points (population, median age, housing units, housing growth, community safety) were corrected on the same date - see the commit message.
